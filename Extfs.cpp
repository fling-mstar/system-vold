...
int Extfs::doMount(const char *fsPath, const char *mountPoint,
    ...
    if (rc == 0) {
        if (chmod(mountPoint,(0777 & ((~permMask) & 0x1ff))) == -1) {
            SLOGE("Fail to chmod %s", mountPoint);
        }

        if (chown(mountPoint, ownerUid, ownerGid) == -1) {
            SLOGE("Fail to chown %s", mountPoint);
        }
    }
    return rc;
}